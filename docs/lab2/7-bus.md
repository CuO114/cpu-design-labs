...
&emsp;&emsp;DRAM总线包含的信号与DRAM IP核的接口信号相同，如表8-1所示。
...
!!! info "AXI总线也是一种系统总线"
    &emsp;&emsp;由表8-1可知，DRAM总线不包含与信号握手、猝发传输相关的信号，因此仅能实现最简单的总线读/写功能。

    &emsp;&emsp;我们在春季学期的计算机组成原理实验学习了AXI总线。AXI总线具有相互独立的读通道和写通道，支持猝发传输和同时处理多个读/写请求，具有高带宽、可扩展性强、生态丰富等优点。Vivado库自带了许多AXI接口的IP核，包括存储器、FIFO、除法器等等。感兴趣的同学可参考计算机组成原理实验的SoC模板工程，利用Vivado库中的IP核，为你的CPU实现AXI总线。
...
``` Verilog title="Bridge.v" linenums="48"
...
``` Verilog title="Bridge.v" linenums="92"
// 根据请求地址返回读数据给CPU
always @(*) begin
    casex (access_bit)
