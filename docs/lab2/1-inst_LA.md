...
!!! danger "注意 :mega:"
    &emsp;&emsp;使用寄存器时需添加`$`符号，如`$r0`、`$zero`等，否则使用LoongSonAssembler汇编工具时将报语法错误。
...
#### *sub.w* (<font color = DodgerBlue>**选**</font>)  
- 指令名：减法  
- 使用语法：`sub.w rd, rj, rk`  
...
#### *bne* (<font color = DodgerBlue>**选**</font>)  
- 指令名：不相等时分支  
...
#### *bge* (<font color = DodgerBlue>**选**</font>)  
- 指令名：大于等于时分支  
