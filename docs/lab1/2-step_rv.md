...
&emsp;&emsp;打开RARS，新建源文件`lab1.asm`，运用miniRV指令集编写题目2的汇编程序。

&emsp;&emsp;代码编写完成后，依次点击菜单栏的 “Run” -> “Assemble” 对代码进行汇编，或点击工具栏中间的汇编按钮进行汇编，如图3-1所示。

<center><img src = "../assets/3-1.png"></center>
<center>图3-1 对编写完成的代码进行汇编</center>
...
&emsp;&emsp;依次点击菜单栏的 “File” -> “Dump Memory”，并在后续弹出的对话框中选择 “Dump Format” 为 “Hexademical Text”，如图3-2所示。

<center><img src = "../assets/3-2.png"></center>
<center>图3-2 以16进制文本格式导出程序的机器码</center>

&emsp;&emsp;点击图3-2中的 “Dump To File” 按钮后，将弹出保存文件的对话框。自行选择合适的文件夹，并为导出的文本文件起名。

&emsp;&emsp;给文件命名时，可将后缀名命名为`.hex`或`.txt`。不管后缀名是什么，所导出的文件都是文本文件，都可以使用常见的文本编辑器打开，比如记事本、VSCode等等。

&emsp;&emsp;打开导出的文本文件，即可查看程序的机器码，如图3-3所示。

<center><img src = "../assets/3-3.png" width = 400></center>
<center>图3-3 查看所导出的程序机器码</center>

!!! tip "关于程序中的数据段 :books:"
    &emsp;&emsp;如果汇编代码没有定义数据段，则程序在汇编后不会生成.data段，否则需要在RARS中分别导出代码段和数据段。具体操作是，在图3-2所示的 “Dump Memory To File” 对话框中，将左侧的 “Memory Segment” 下拉选择成 “.data” 后，再点击 “Dump To File” 按钮即可。此时，所导出的程序将由两个`.hex`文件组成，一个存储代码段，另一个存储数据段。
...
&emsp;&emsp;打开Logisim工具，并依次点击菜单栏的 “File” -> “Open”，打开 RISC-V_SoC.circ 的SoC电路设计文件。该 SoC 电路包含RISC-V单周期CPU、指令存储器、数据存储器以及外设接口电路和外设，如图3-4所示。

<center><img src = "../assets/3-4.png"></center>
<center>图3-4 RISC-V SoC顶层电路图</center>

!!! example "外设的编址方式 :computer:"
    &emsp;&emsp;在本课程中，存储器和外设统一编址，即存储器和外设共用整个32位地址空间，其中最高的64KB (`0xFFFF0000` ~ `0xFFFFFFFF`) 用作I/O地址空间。

&emsp;&emsp;在图3-4所示的RISC-V SoC电路中，各模块的层次关系为：
...
&emsp;&emsp;在Logisim左侧的模块列表中，找到`IROM`模块，双击打开其电路。然后在`Program_ROM`存储器上点击右键选择`Clear Contents`，然后再次右键选择`Edit Contents`，如图3-5所示。

<center><img src = "../assets/3-5.png" width = 650></center>
<center>图3-5 清空指令存储器</center>

&emsp;&emsp;打开1.2小节的`.hex`文件，将其内容复制并粘贴到IROM存储器的编辑框中，从而将程序的机器码指令存放到指令存储器的0地址处，如图3-6所示。

<center><img src = "../assets/3-6.png" width = 100%></center>
<center>图3-6 将`.hex`文件中的机器码放置到IROM</center>

&emsp;&emsp;粘贴完成后，关闭图3-6右侧的Hex Editior窗口。
...
&emsp;&emsp;在程序运行过程中，SoC电路的信号将实时发生变化。电路运行过程中，可使用左上角工具栏的戳工具点击信号线，从而查看某个信号的取值，如图3-7所示。

<center><img src = "../assets/3-7.png" width = 100%></center>
<center>图3-7 程序执行时，各信号根据当前执行的指令发生相应的变化</center>

&emsp;&emsp;调试时，可通过断点调试模块设置断点，如图3-8所示。操作方法是，使用编辑工具选中调试模块的`breakPC`常量输入，然后在左下方窗口将其修改为断点指令的PC值。设置断点后，SoC电路将在断点指令处暂停运行。点击图3-8中的`continue`按钮，SoC电路即可继续执行后续指令。

<center><img src = "../assets/3-8.png" width = 500></center>
<center>图3-8 设置断点指令的地址</center>

&emsp;&emsp;程序调试完成后，可将时钟频率调高，从而加快运行速度。操作方法是，依次点击菜单栏的 “Simulate” -> “Tick Frequency”，将时钟频率设置到最高的4.1kHz，如图3-9所示。

<center><img src = "../assets/3-9.png" width = 450></center>
<center>图3-9 修改时钟频率</center>

!!! info "SoC电路的例外与中断 :feet:"
    &emsp;&emsp;SoC电路支持 **ecall** 系统调用和 **外部中断** 两种例外的处理。例外处理程序存放在IROM模块的`Trap_Handler_ROM`存储器中。

    &emsp;&emsp;**例外处理程序的统一入口地址是`0x1C090000`**。发生例外时，例外处理程序将根据例外的原因作出相应的处理 —— 如果是`ecall`指令，例外处理程序将在数码管和LED上显示寄存器`a0`的值；如果是外部中断（即图3-7所示的button按钮），例外处理程序将在数码管和LED上显示`0x11111111` ~ `0x88888888`。
