...
!!! warning "注意 :teacher:"
    &emsp;&emsp;^^程序最终要在FPGA开发板上用你实现的CPU来运行和演示^^，因此 **编写程序时，不要使用你不准备实现的指令**。比如，后续设计CPU如果不实现16条附加指令，则编写程序时，只使用<a href="../1-step_rv/#21-risc-v-soc" target=_blank>实验步骤 - 2.1节 - 表2-1</a>所示的21条必做指令。

    &emsp;&emsp;同理，如果使用miniLA指令集，编写程序时也 **不要使用你不准备实现的指令**。如果后续设计CPU时不实现附加指令，则编写本题目时，只使用<a href="../1-step_la/#21-loongarch-soc" target=_blank>实验步骤 - 2.1节 - 表2-a</a>所示的22条指令。
