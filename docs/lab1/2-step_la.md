&emsp;&emsp;开始之前，从首页的课程材料下载链接，下载实验1所需的相关文件，包括示例程序、汇编工具、Logisim、SoC电路文件等。
...
=== "LoongSonAssembler环境"
    ...
    &emsp;&emsp;新建源文件`lab1.asm`，在LoongSonAssembler或代码编辑器编写题目2的汇编程序。

    &emsp;&emsp;程序编写完成后，在LoongSonAssembler打开`lab1.asm`，点击左侧工具栏的蓝色齿轮按钮（或按下快捷键 ++f5++）即可对代码进行汇编，如图3-a所示。

    <center><img src = "../assets/3-a.png" width = 600></center>
    <center>图3-a 使用LoongSonAssembler对代码进行汇编</center>
...
=== "LA32R GNU + NEMU环境"
    ...
    &emsp;&emsp;接着，使用第三方代码编辑器新建汇编源文件`lab1.asm`并编写计算器程序。
    ...
    &emsp;&emsp;如果编写的汇编程序包含数据段，则sh脚本将生成`.bin`、`_text.hex`和`_data.hex`三个文件。其中，`_text.hex`和`_data.hex`文件分别是程序的代码段和数据段的十六进制机器码文本文件，而`.bin`文件则同时包含了代码段和数据段，其结构如图3-I所示。

    <center><img src = "../assets/3-I.png" width = 200></center>
    <center>图3-I 程序包含数据段时，`.bin`文件的结构</center>

    &emsp;&emsp;由图3-I可知，汇编程序的 **代码段将存放在`.bin`文件的`0`偏移地址处**，而汇编程序的 **数据段则存放在`.bin`文件的`0x00010000`偏移地址处**。
...
=== "LoongSonAssembler环境"
    ...
    &emsp;&emsp;依次点击菜单栏的 “Result” -> “另存为*.txt”，并在某个文件夹下将导出的机器码另存为`.hex`文本文件，如图3-b所示。

    <center><img src = "../assets/3-b.png" width = 700></center>
    <center>图3-b 导出汇编程序的机器码</center>

    &emsp;&emsp;给文件命名时，可将后缀名命名为`.hex`或`.txt`。不管后缀名是什么，所导出的文件都是文本文件，都可以使用常见的文本编辑器打开，比如记事本、VSCode等等。

    &emsp;&emsp;打开导出的文本文件，即可查看程序的机器码，如图3-c所示。

    <center><img src = "../assets/3-c.png" width = 320></center>
    <center>图3-c 查看所导出的程序机器码</center>

    !!! tip "关于程序中的数据段 :books:"
        &emsp;&emsp;如果汇编代码没有定义数据段，则代码在汇编后不会生成.data段，否则需要在LoongSonAssembler中分别导出代码段和数据段。具体操作是，在图3-b所示的界面中，依次点击 “Data” -> “另存为*.txt” 即可。此时，所导出的程序将由两个`.hex`文件组成，一个存储代码段，另一个存储数据段。
...
&emsp;&emsp;打开Logisim工具，并依次点击菜单栏的 “File” -> “Open”，打开 LoongArch_SoC.circ 的SoC电路设计文件。该 SoC 电路包含LoongArch单周期CPU、指令存储器、数据存储器以及外设接口电路和外设，如图3-d所示。

<center><img src = "../assets/3-d.png"></center>
<center>图3-d LoongArch SoC顶层电路图</center>

!!! example "外设的编址方式 :computer:"
    &emsp;&emsp;在本课程中，存储器和外设统一编址，即存储器和外设共用整个32位地址空间，其中最高的64KB (`0xFFFF0000` ~ `0xFFFFFFFF`) 用作I/O地址空间。

&emsp;&emsp;在图3-d所示的LoongArch SoC电路中，各模块的层次关系为：
...
&emsp;&emsp;在Logisim中双击打开LoongArch SoC的`IROM`模块，在IROM的存储器上点击右键选择`Clear Contents`，然后再次右键选择`Edit Contents`，如图3-e所示。

<center><img src = "../assets/3-e.png" width = 650></center>
<center>图3-e 清空指令存储器</center>

&emsp;&emsp;打开1.2小节的`.hex`文件，将其内容复制并粘贴到IROM存储器的编辑框中，从而将程序的机器码指令存放到指令存储器的0地址处，如图3-f所示。

<center><img src = "../assets/3-f.png" width = 100%></center>
<center>图3-f 将`.hex`文件中的机器码放置到IROM</center>

&emsp;&emsp;粘贴完成后，关闭图3-f右侧的Hex Editior窗口。

!!! info "如果你的程序有数据段 :bookmark_tabs:"
    &emsp;&emsp;若所编写的汇编程序除了代码段，还含有数据段，则需注意，SoC电路的数据段基地址为`0x00000000`，请自行检测是否需要修改程序中的访存地址。
    
    &emsp;&emsp;确认数据段地址无误后，参考上述操作，将数据段导出的`.hex`文件的内容放置到DRAM模块的存储器中。
...
&emsp;&emsp;在程序运行过程中，SoC电路的信号将实时发生变化。电路运行过程中，可使用左上角工具栏的戳工具点击信号线，从而查看某个信号的取值，如图3-g所示。

<center><img src = "../assets/3-g.png"></center>
<center>图3-g 程序执行时，各信号根据当前执行的指令发生相应的变化</center>

&emsp;&emsp;调试时，可通过断点调试模块设置断点，如图3-h所示。操作方法是，使用编辑工具选中调试模块的`breakPC`常量输入，然后在左下方窗口将其修改为断点指令的PC值。设置断点后，SoC电路将在断点指令处暂停运行。点击图3-h中的`continue`按钮，SoC电路即可继续执行后续指令。

<center><img src = "../assets/3-h.png" width = 500></center>
<center>图3-h 设置断点指令的地址</center>

&emsp;&emsp;程序调试完毕后，可将SoC电路的时钟频率调高，从而使得CPU运行得更快。依次点击菜单栏的 “Simulate” -> “Tick Frequency”，将时钟频率设置到最高的4.1kHz，如图3-i所示。

<center><img src = "../assets/3-9.png" width = 450></center>
<center>图3-i 修改时钟频率</center>
