...
!!! tip "参考实现 :sparkles:"
    ...
    &emsp;&emsp;特别注意的是，此处增加`id_rf1`和`id_rf2`信号是为了避免I型、U型和J型指令执行时出现RAW冒险的误判。此外，零号寄存器不可写，因而不存在RAW冒险，故而需要排除之。
...
&emsp;&emsp;流水线暂停是功能最强大的冒险解决方法，因此它将流水线中并行进行的操作串行化。暂停法除了可以解决数据冒险，还可用于实现访存指令和乘除法指令。

!!! tip "暂停法实现访存指令、乘除法指令的思路"
    &emsp;&emsp;访存指令需要通过总线读写主存数据，需要多个时钟周期才能完成。乘除法指令需要通过硬件乘除法器完成运算，而硬件乘除法也需要多个时钟周期才能完成。对于32bit的“IF-ID-EX-MEM-WB”的五级流水CPU，假设采用两位编码的Booth算法实现乘法器，乘法操作至少也需要16个时钟周期才能完成。因此，在不改变流水线深度的前提下，需要借助流水线暂停来延长执行阶段，才能实现乘除法指令。

    &emsp;&emsp;使用暂停法实现访存指令的思路是：在EX阶段或MEM阶段通过相应的控制信号（比如`ram_rop`、`ram_wop`等）判断指令是否是访存指令；如果是，则通过MREQ模块产生只有效一个时钟周期的访存请求信号（即`daccess_ren`或`daccess_wen`），同时暂停流水线的IF阶段和ID阶段，直到访存完成（即`daccess_valid`或`daccess_wresp`有效），再令流水线恢复执行。
    
    &emsp;&emsp;使用暂停法实现乘除法指令的思路是：在EX阶段通过`alu_op`判断指令是否是乘除法指令；如果是，则产生只有效一个时钟周期的乘除法启动信号`start`，同时暂停流水线的IF阶段和ID阶段，直到乘除法器输出的`busy`信号从高电平变成低电平，此时乘除法运算完成，再令流水线恢复执行。
...
&emsp;&emsp;添加了流水线暂停机制的流水线CPU数据通路形如图4-8所示。

<center><img src = "../assets/4-8.png"></center>
<center>图4-8 具有暂停功能的流水线CPU数据通路示例</center>

!!! hint "参考实现 :sparkles:"
    &emsp;&emsp;图4-8中的虚线表示使用生成的暂停信号暂停流水线寄存器。限于空间，具体的暂停逻辑未画出。对于IF/ID寄存器，其暂停逻辑的参考RTL实现如下列代码所示。
...
!!! example "举个栗子 :chestnut:"
    ...
    &emsp;&emsp;对于图4-1所示的RAW冒险，可采用数据前递方法解决 —— 将执行阶段、访存阶段或写回阶段的中间结果前递给译码阶段，并让前递而来的数据参与到指令译码时的源操作数选择，如图4-9所示。

    <center><img src = "../assets/4-9.png" width = 600></center>
    <center>图4-9 数据前递示意图</center>

    &emsp;&emsp;图4-9中，从EXE指向ID、从MEM指向ID以及从WB指向ID的三条紫色箭头，分别对应图4-1中RAW冒险的三种情形。第一条指令在执行阶段即可产生计算结果（x19寄存器的新值），此时通过执行阶段到译码阶段的数据前递，可让第二条指令在译码时获得x19的新值，从而在不需暂停流水线的情况下解决了第一、二条指令之间的RAW冒险。

&emsp;&emsp;添加了数据前递机制的流水线CPU数据通路形如图4-10所示。

<center><img src = "../assets/4-10.png"></center>
<center>图4-10 增加数据前递机制的流水线CPU数据通路</center>

!!! hint "参考实现 :sparkles:"
    &emsp;&emsp;译码阶段被前递而来的数据，需要加入到源操作数的选择逻辑当中，相应的RTL参考实现如下列代码所示。
