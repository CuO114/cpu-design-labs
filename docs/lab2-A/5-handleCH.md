&emsp;&emsp;按照RISC-V和LoongArch的指令集架构规范，控制冒险最简单的解决方法是 **默认预测所有指令都不发生跳转**。只有在EX阶段根据运算结果判断 **指令需要跳转时，再清除IF阶段和ID阶段，并使用正确的PC重新取指**。显然，在指令发生跳转时，会造成流水线性能的损失。
...
&emsp;&emsp;分支延迟槽一定程度上降低了汇编程序的可读性。此外，随着流水线深度的增加，延迟槽的实现也变得更加复杂，但其带来的性能提升较为有限。为了简化设计和节省芯片面积，RISC-V和LoongArch均已抛弃分支延迟槽技术，而是直接通过分支预测解决控制冒险问题。
...
&emsp;&emsp;**在本课程中，实现默认预测不跳转的静态预测即可**。感兴趣的同学可尝试实现动态预测方法。
